"""Erzeugt Cooling-Diagramme aus aufbereiteten Raumdaten.
...
Wichtige Annahmen:
    Die Cooling-Zeitreihe steht in der Spalte ``zone_energy_q_cool``. Fuer die
    Darstellung werden Kuehlleistungen negativ nach unten geplottet.
"""
...
REQUIRED_COOLING_COLUMN = "zone_energy_q_cool"
...
def style_technical_axis(ax, title, subtitle, axis_config, series_count, show_legend, legend_y_anchor=-0.18):
    """Formatiert das Hauptdiagramm mit negativer Cooling-y-Achse."""
    ax.set_facecolor(TECHNICAL_PLOT_BG)
    ax.set_title(title, loc="left", fontsize=13, fontweight="bold", color=TECHNICAL_TEXT_COLOR, pad=10)
    ax.text(
        1.0,
        1.08,
        subtitle,
    ...
    ax.set_ylabel("Kuehllast [W]", fontsize=10, color=TECHNICAL_TEXT_COLOR)
    ax.xaxis.tick_top()
...
def get_room_hourly_data(csv_file, debug=False):
    """Lädt die stündliche q_cool-Zeitreihe eines Raums."""
    ...
    if "time" not in df.columns:
        ...
        return None
    df = df[["time", REQUIRED_COOLING_COLUMN]].copy()
    df["time"] = pd.to_numeric(df["time"], errors="coerce")
    df = df.dropna(subset=["time", REQUIRED_COOLING_COLUMN])
    if df.empty:
        return None
...
def plot_yearly_single_variant(room_data, variant_name, output_dir, debug=False):
            ...
            pd.DataFrame(
                {
                    "time": df["time"],
                    "room": room,
                    "q_cool": df[REQUIRED_COOLING_COLUMN],
                }
            )
        )
    ...
    draw_technical_line_plot(
        ...
        x_col="time",
        group_col="room",
        title=f"Kuehllastverlauf Raeume - {variant_name}",
...
def plot_monthly_single_variant(room_data, variant_name, output_dir, month_name, debug=False):
    ...
    rows = []
    for room, df in room_data.items():
        temp = filter_time_window(df[["time", REQUIRED_COOLING_COLUMN]].copy(), time_window)
        if temp.empty:
            continue
        temp["room"] = room
        temp["q_cool"] = temp[REQUIRED_COOLING_COLUMN]
        rows.append(temp[["time_window", "room", "q_cool"]])
...
def plot_single_room_time_series(
    ...
    if view == "year":
        plot_df = pd.DataFrame(
            {
                ...
                "series": room,
                "q_cool": room_df[REQUIRED_COOLING_COLUMN],
            }
        )
        axis_config = build_time_axis_config("year")
        subtitle = build_plot_subtitle("year")
        ...
        filtered = filter_time_window(room_df[["time", REQUIRED_COOLING_COLUMN]].copy(), time_window)
        if filtered.empty:
            if debug:
        ...
        plot_df = pd.DataFrame(
            {
                ...
                "series": room,
                "q_cool": filtered[REQUIRED_COOLING_COLUMN],
            }
        )
    ...
    draw_technical_line_plot(
        ...
        x_col="time_axis",
        group_col="series",
        title=f"Kuehllastverlauf - {variant_name} - {room}",
        subtitle=subtitle,
...
def plot_yearly_variant_comparison(room_data_by_variant, rooms, output_dir, debug=False):
    ...
    for room in rooms:
        ...
        draw_technical_line_plot(
            ...
            x_col="time",
            group_col="variant",
            title=f"Kuehllastvergleich Varianten - {room}",
            subtitle=build_plot_subtitle("year"),
            axis_config=build_time_axis_config("year"),
            output_file=plot_file,
        )
...
def plot_monthly_variant_comparison(room_data_by_variant, rooms, output_dir, month_name, debug=False):
    ...
    for room in rooms:
        ...
        for variant_name, room_data in room_data_by_variant.items():
            ...
            temp["q_cool"] = temp[REQUIRED_COOLING_COLUMN]
            rows.append(temp[["time_window", "variant", "q_cool"]])
...
def load_room_csv(csv_file, debug=False):
    """Laedt Raumdaten und spiegelt q_cool fuer die negative Darstellung."""
    try:
        df = pd.read_csv(csv_file)

        if REQUIRED_COOLING_COLUMN not in df.columns:
            print(f"    X Fehlende Spalte {REQUIRED_COOLING_COLUMN} in {csv_file}")
            return None

        # Robuste numerische Konvertierung
        df[REQUIRED_COOLING_COLUMN] = pd.to_numeric(df[REQUIRED_COOLING_COLUMN], errors="coerce")
        df = df.dropna(subset=[REQUIRED_COOLING_COLUMN])
        df[REQUIRED_COOLING_COLUMN] = -df[REQUIRED_COOLING_COLUMN].abs()
...
def get_variant_data(variant_dir, debug=False, rooms=None):
    """Laedt fuer die Balkenansicht die maximale negative Kuehlleistung je Raum."""
    if rooms is None:
        rooms = ROOMS
    ...
    for room in rooms:
        ...
        df = load_room_csv(csv_file, debug=debug)
        if df is not None:
            q_cool = df[REQUIRED_COOLING_COLUMN].dropna()
            if q_cool.empty:
                continue
            max_q_cool = -abs(q_cool).max()
            data[room] = max_q_cool
            if debug:
                print(f"    {room}: Max q_cool = {max_q_cool}")
    return data
...
def main(
    ...
    """Laedt Varianten- und Raumdaten und erzeugt die angeforderte Cooling-Ausgabe."""
    if not os.path.exists(datenbank_dir):
        print(f"X Verzeichnis mit aufbereiteten Daten nicht gefunden: {datenbank_dir}")
        return

    print("=" * 70)
    print("KUEHLVERGLEICH - Vergleichsdiagramme")
    print("=" * 70)
    ...
    resolved_run_id = get_run_id("cooling_comparison", run_id=run_id)
    created_count = 0
    ...
    if view == "bar":
        for variant_name, variant_path in variants:
            ...
            all_data = [{"room": room, "max_q_cool": max_q_cool} for room, max_q_cool in data.items()]
            ...
            ax.set_xlabel("Raum")
            ax.set_ylabel("Kuehlleistung [W]")
            ax.grid(True, axis="y", color=TECHNICAL_GRID_COLOR, linewidth=0.8, alpha=0.8)
            ax.set_axisbelow(True)
            ax.tick_params(axis="x", rotation=45)
            min_value = df_plot["max_q_cool"].min()
        ...
        if variant_mode == "single":
            ...
            if len(variants) == 1:
                ...
                print("=" * 70)
                print(f"Kuehlvergleich abgeschlossen: {created_count} Plots erstellt")
                print("=" * 70)
                return
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Vergleicht Kuehlleistungen (q-cool) der Varianten.")
    parser.add_argument(
        "--datenbank_dir",
